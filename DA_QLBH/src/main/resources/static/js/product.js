...
async function loadProductIndex(page) {
    ...
    for (i = 0; i < list.length; i++) {
        main += `<div class="col-lg-20p col-md-3 col-sm-6 col-6">
                    <a href="detail?id=${list[i].ProductID}" class="linkpro">
                        <div class="singlepro">
                            <img src="${list[i].ImageURL}" class="imgpro">
                            <span class="proname">${list[i].Name}<br>Màu: ${list[i].Color} - Size: ${list[i].Size}</span>
...
async function loadAProduct() {
    ...
    if (id != null) {
        ...
        document.getElementById("imgdetailpro").src = result.ImageURL
        document.getElementById("descriptiondetail").innerHTML = result.Description
        document.getElementById("colorname").innerHTML = result.Color
